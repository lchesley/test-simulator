...
{
    public class BrandOfIce : IAction
    {
        //Private variables.
        ...
        {
            get { return ElementalAffinity.Ice; }
        }
        ...
        public string ExecuteAction(ICraft craft, ICrafter crafter, List<IModifier> activeModifiers, Random rng, Conditions condition)
        {
            ...
            if (Calc.ActionIsSuccessful(rng, SuccessChance))
            {
                ...
                if (activeModifiers.Where(o => o.ModifierName == "Name Of Ice").Count() > 0)
                {
                    progressModifier = 2 * (craft.Progress / craft.Difficulty - 1) + 3;
                }

                if(craft.Affinity == ElementalAffinity.Ice)
                {
